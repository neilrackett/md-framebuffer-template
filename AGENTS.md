...
- **Universal sampled sound** — 25 kHz 8-bit STE DMA sound where the machine has it, ~6 kHz 6-bit YM2149 on a plain ST, detected at boot and switched at runtime. You supply one unsigned-8-bit PCM asset.
...
The RP fills a 1 KB cart buffer at `$FA4100` once per VBL; **which back-end drains it is decided at runtime**. `userfw.s` checks the `_SND` cookie at boot (bit 1 = DMA/PCM sound), stores the answer in `UFW_HAS_DMA`, and reports it to the RP over the cart bus every VBL so the RP produces the matching format. Both paths are compiled in — there is no build-time choice.

| | STE / Mega STE / TT | plain ST / Mega ST |
| --- | --- | --- |
| Back-end | DMA sound chip | YM2149 via Timer-B IRQ |
| Format | 25,033 Hz mono, 8-bit **signed** | 5,585 Hz (vA, vB) volume pairs |
| Per VBL | ~500 B, steered to the chip | 224 B |
| m68k cost | ~0.3 ms (buffer refill) | ~1.57 ms (~8% of the VBL) |

1. **Build-time asset conversion** (`tools/wav_to_ym4.py`). Source `.SAM` / `.WAV` is resampled and emitted as **unsigned 8-bit mono PCM** (`--mode raw-byte`, the default; silence = `0x80`) into `rp/src/include/audio_sample.h` — a C array embedded in the RP firmware — and/or a `.YMS` file for SD streaming (16-byte header, mode tag 3). PCM is the one body format the firmware can cook for *both* back-ends, so it's the only one `audio_play_yms_file()` accepts. `assets/demo_jingle.sam` is the source of the bundled jingle. The pre-cooked YM modes (`dual-ghost`, `single-a`, `best-pair`, `nibble`, `ghostbusters`) remain in the tool for m68k handlers that play them directly, but cannot feed DMA sound.
2. **RP cooks + fills the cart buffer per VBL** (`rp/src/audio.c`). `audio_render_frame()` paces to ~50 Hz via `time_us_32` and refills `CART_AUDIO_BUFFER_OFFSET` (`$FA4100`) with 500 B (DMA) or 224 B (YM) — `audio_set_mode()`, driven by the cart-bus report decoded in `fb.c`, picks the size. `audio_play_loop()` / `audio_play_yms_file()` pull one VBL of source PCM (rate-agnostic, with a Q16 accumulator so a fractional samples-per-VBL rate doesn't drift) and `audio_emit()` cooks it: sign-flip passthrough for DMA, box-average + Ghostbusters LUT for YM. Apps that cook their own audio install `audio_set_fill_callback()` and switch on `audio_get_mode()`. The Ghostbusters LUT cherry-picks 64 of the ~135 distinct acoustic sums achievable from the YM's log volume curve, giving ~6 effective bits vs the 4-bit ceiling of single-channel playback.
3. **m68k STE DMA path** (`.boot_audio_dma` / `.after_copy` / `userfw_snd_irq` in `userfw.s`). Mono 25 kHz loop-mode DMA over a **double buffer in the unused tails of the two screen pages** (`$77D00` / `$7FD00`, `STE_SND_COPY` = 512 B each — past the 32000-byte framebuffer, below the userfw scratch slots, never displayed and never touched by the FB blit, so it costs no separately-allocated ST RAM). Each VBL the m68k reads the DMA frame counter (`$FFFF890B`/`0D`) to see which buffer the chip is in, and copies the cart buffer into the *other* one — so the write never crosses the DMA read pointer (single-buffering tore one sample per VBL = a constant ~50 Hz buzz). Two details make it stable over hours rather than minutes:
    - **START/END are written from a Timer-A IRQ, not the VBL loop.** The chip latches them only when it reaches END, and the frame end pulses MFP Timer-A's event input (event-count mode, TADR=1), so `userfw_snd_irq` re-points them the instant the chip switches buffers — a whole frame before the next latch. Written from the VBL loop the six byte writes sat at an arbitrary phase of the DMA frame, and the chip's slow drift eventually latched a START from one buffer with an END from the other, playing the 32 KB of screen memory between them (~1.3 s of noise, every few minutes).
    - **The played length is measured, not assumed.** The DMA and the video run off different oscillators, so the chip eats ~501.5 samples per frame, not 500, and the figure differs per machine. `UFW_SND_LEN` (480..512) is steered from the sign of the frame-counter offset's drift, sampled every VBL and nudged every fourth, and reported to the RP at `SNDLEN_WINDOW_BASE` (`$FB8C00` + len − 480) so the RP resamples onto exactly that many samples. A fixed 500 ran the buffer dry about every 350 frames and replayed one — the ~7-second crackle. The copy stays a fixed 512 B; anything past the length sits unplayed.

    DMA is stopped before returning to GEM.
4. **m68k VBL handler resets the audio cursor** (`userfw_vbl` in `userfw.s`). On every vsync the handler does `movea.l #AUDIO_BUFFER_ADDR, a0` + `clr.w UFW_VBL_FLAG.w` + `rte`. A0 is the **dedicated Timer-B audio read cursor** — kept out of `FBDRV_INLINE`'s MOVEM list so the IRQ handler doesn't have to save/restore it. Skipped on the DMA path, which has no read cursor. The VBL reset is the sync edge: m68k always reads from buffer[0..~223], RP always rewrites buffer[0..1023] each VBL, drift is bounded by the headroom. This replaced the originally-planned A/B double-buffer + `AUDIO_CHUNK_COUNTER` mechanism — VBL-edge resync is simpler and just as click-free.
5. **m68k Timer-B handler plays the pair** (`userfw_timerb_audio`). MFP Timer-B is in /4 delay mode with TBDR=110 → 5,585.45 Hz, ~112 fires per PAL VBL. Each fire: `move.b (a0)+, $FFFF8802.w` (vA → ch A vol, reg 8 latched on entry) → `move.b #9, $FFFF8800.w` (latch reg 9) → `move.b (a0)+, $FFFF8802.w` (vB → ch B vol) → `move.b #8, $FFFF8800.w` (re-latch reg 8 for next fire) → `rte`. ~68 cyc handler body + ~44 cyc IRQ entry/exit = ~112 cyc/fire = ~1.57 ms/VBL = ~8% of the PAL VBL budget. MFP is in **auto-EOI mode** (VR S=0) so the in-service bit clears automatically per IACK cycle — no software EOI needed in the handler.
6. **YM boot state** — boot enables channels A + B in the mixer (`MIXER = $FC`, tones on, noise off, periods=0 for a DC pass-through), zeroes both volumes, then latches reg 8 so the first Timer-B fire writes ch A directly.

Tuning knobs:
- `TIMERB_COUNT` in `userfw.s` controls the YM rate. Raise to slow (longer per-VBL audio slack), lower to speed (better fidelity at higher cost). Must match `AUDIO_FILL_BYTES_YM` in `rp/src/audio.c` (= 2 × fires-per-VBL) — otherwise the m68k read cursor drifts. The DMA length needs no such pairing: it is measured and reported at runtime, but `STE_SND_LEN_MIN` in `userfw.s` and `AUDIO_SNDLEN_BIAS` in `audio.c` must stay equal, since the report is biased by it to fit a byte.
- Source assets are **rate-agnostic** — `audio_play_loop(pcm, bytes, rate_hz)` and the `.YMS` header's rate field tell the RP what it's resampling from, so changing `TIMERB_COUNT` no longer re-pitches the jingle. Generate at `--target-rate 25033` (the default) to get the STE's full quality; the YM path box-averages it down.
- Build the m68k side with `-DFORCE_NO_DMA=1` to force detection to report "no DMA", so the YM fallback can be exercised on an STE.
- Audio still uses **no separately-allocated Atari RAM** — the YM path keeps all state in registers (A0), and the DMA path borrows the two screen-page tails the shifter never displays.
...
| `$FA4100` | `AUDIO_BUFFER_ADDR` | 1024 B | Audio buffer, refilled by the RP every VBL in the live back-end's format: 500 B of signed 8-bit PCM for STE DMA (the m68k copies it into the screen-tail double buffer), or 224 B of Ghostbusters-LUT (vA, vB) pairs at ~5,585 Hz for YM (the m68k VBL handler resets A0 to the base every frame). The rest is headroom for VBL / Timer-B drift. |
